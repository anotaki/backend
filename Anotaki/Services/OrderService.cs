...
namespace anotaki_api.Services
{
    public class OrderService(AppDbContext context, IOrderPublisher orderPublisher) : IOrderService
    {
        ...
        {
            var cart = await _context.Orders.Include(x => x.Items).FirstOrDefaultAsync(o => o.UserId == userId && o.OrderStatus == OrderStatus.Cart);
        ...
        public async Task CheckoutOrder(Order order, CheckoutOrderDTO dto)
        {
            ...
            var address = await _context.Addresses.FindAsync(dto.AddressId) ?? throw new Exception($"Address with id {dto.AddressId} not found.");
        ...
        public async Task<Order> AddProductToOrder(Order order, User user, AddProductToOrderDTO dto)
        {
            ...
            var productItem = new OrderProductItem()
            {
                ...
                ExtrasItems = dto.Extras.Select(x => new OrderExtraItem()
                {
                    ExtraId = x.ExtraId,
                    Quantity = x.Quantity,
                }).ToList(),
